...
class ConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_discovery_confirm(
        ...
        if user_input is not None:
            _LOGGER.error("Async_step_discovery_confirm() %s", user_input)
            _LOGGER.error("Discovered_device %s", self._discovered_device)
    ...
    async def async_step_pick_device(
        ...
        if user_input is not None:
            ...
            _LOGGER.error(
                ...
                self._discovered_devices[user_input["device"]].__dict__,
            )
        ...
        _LOGGER.error("Configured Devices: %s", self._async_current_entries())
        self._discovered_devices = await async_discover_devices(self.hass)
        _LOGGER.error("Discovered Devices: %s", self._discovered_devices)
        devices_name = {
